...
(function ($, document, window) {
	...
	function launch(target) {
		if (!closing) {
			...
			if (!open) {
				...
				$box.attr({'class':settings.styleCbox1});			
				...
				$overlay.attr({'class':settings.styleOverlay});
				$overlay.css({
	...
	function appendHTML() {
		if (!$box && document.body) {
			...
			$loadingBay = $tag(div, false, 'position:absolute; width:100%; visibility:hidden; display:none');
